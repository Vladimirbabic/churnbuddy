  ...
  if (bgColor) {
    return (
      <>
        {/* Light mode */}
        <div
          className="flex items-center justify-center shrink-0 p-0.5 dark:hidden"
          style={{
        ...
        <div
          className="hidden dark:flex items-center justify-center shrink-0 p-0.5"
          style={{
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {WORKSPACE_ITEMS.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.href}>
                    <SidebarMenuButton asChild isActive={isActive} tooltip={item.label} className="py-0.5">
                      <Link href={item.href}>
                        {item.customIcon ? (
